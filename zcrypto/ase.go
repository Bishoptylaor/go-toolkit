...
func AESEncryptWithSalt(origData, key []byte, iter int, magic string, h func() hash.Hash, pad Pad, f func(origData, key, iv []byte, pad Pad) ([]byte, error)) ([]byte, error) {
	...
	var salt, _ = randBytes(pkc5SaltLen)
	var sKey = pbkdf2.Key(key, salt, iter, len(key), h)
	var sIV = pbkdf2.Key(sKey, salt, iter, maxIvLen, h)
...
func AESGCMEncrypt(origData, key, additional []byte) ([]byte, error) {
	...
	nonce, err := randBytes(blockMode.NonceSize())
	if err != nil {
		return nil, err
	}

	return blockMode.Seal(nonce, nonce, origData, additional), nil
}
